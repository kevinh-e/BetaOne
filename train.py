...
from self_play import SelfPlayData  # Import the data structure type hint
...
class ChessDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure tensors are correctly typed (policy might be numpy)
        policy_tensor = torch.from_numpy(policy).float()
        value_tensor = torch.tensor(
            [value], dtype=torch.float32
        )  # Value needs to be a tensor
...
def calculate_loss(
    ...
    # Use log_softmax on logits and negative log likelihood loss (NLLLoss)
    # Or directly use CrossEntropyLoss which combines log_softmax and NLLLoss
    policy_loss = F.cross_entropy(policy_logits, target_policy)

    # Total Loss: Sum of value and policy losses
    # AlphaZero paper doesn't mention weighting, but sometimes used.
...
def train_network(
    ...
    for batch_idx, (states, target_policies, target_values) in enumerate(data_loader):
    ...
    return avg_total_loss  # Return average loss for the epoch
...
def run_training_iteration(
    ...
    # --- Load Data ---
    # Load data from the most recent self-play iteration(s)
    # Can load from multiple past iterations if desired
    training_data = load_data_from_iteration(
        iteration
    )  # Load from the corresponding self-play iter
    ...
    # Optional: Limit buffer size (e.g., keep only last N games or examples)
    if (
    ...
    data_loader = DataLoader(
        ...
        pin_memory=torch.cuda.is_available(),  # Use pinned memory if using GPU
    )
